...
class NuclearSubtypeDataset(Dataset):
    ...
    def process(self):
        ...
        """
        idx_id = defaultdict(str)
        for idx, matrix_file in enumerate(self.retrieve_file_path(f'{self.root}/raw', 'csv')):
            idx_id[int(idx)] = matrix_file
            if os.path.exists(f'{self.root}/processed/graph_{idx}.pt'):
                pass
            else:
    ...
    def get(self, idx):
        """
            get() generates a torch_geometric.data.data.Data() object of 'root/processed/graph_{idx}.pt'
